...
class MetricsQueriesMixin:
    ...
    def usage_summary(self, since_iso: str, until_iso: str | None = None) -> dict[str, Any]:
        ...
        (dimension × model) so each model's price applies correctly."""
        ...
        by_purpose = [
            {"purpose": p, **v}
            for p, v in fold(grouped("purpose AS k1, ", "purpose, model"), "k1").items()
        ]
        by_day = [
            {"date": d, **v}
            for d, v in sorted(
        ...
        # topics need names joined; NULL topic_id → background/system spend.
